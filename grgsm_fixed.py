...
class grgsm_livemon_headless(gr.top_block):
    ...
    def __init__(self, args="", collector="localhost", collectorport='4729', fc=925.8e6, gain=30, osr=4, ppm=0, rec_len=1000000, samp_rate=2000000.052982, serverport='4729', shiftoff=400e3):
        ...
        self.rtlsdr_source_0 = _soapy.source("driver=hackrf", "fc32", 1, args, "", [""], [""])
        self.rtlsdr_source_0.set_sample_rate(0, samp_rate)
        self.rtlsdr_source_0.set_frequency(0, fc)
        self.rtlsdr_source_0.set_frequency_correction(0, 0)
        self.rtlsdr_source_0.set_gain_mode(0, False)
        # Use explicit HackRF element gains for best GSM reception
        try:
            self.rtlsdr_source_0.set_gain(0, "RF", min(gain, 40))  # LNA
            self.rtlsdr_source_0.set_gain(0, "IF", min(gain, 40))  # VGA
        except Exception:
            self.rtlsdr_source_0.set_gain(0, gain)
        self.rtlsdr_source_0.set_bandwidth(0, samp_rate)
